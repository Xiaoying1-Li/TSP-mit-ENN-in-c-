...
vector<F32vec4> MyWidget::circle_generate(float center_x,float center_y) {
...
vector<F32vec4> MyWidget::normalize(vector<F32vec4> C) {
    float min_x = 100000;
    float max_x = -100000;
    float min_y = 100000;
    float max_y = -100000;
...
void MyWidget::iterration() {
    ...
    for (int i = 0;i < net.size();i++) {
        ...
        for (int j = 0;j < C.size();j++) {
                        delta=delta+ v[j][i] * (C[j] - net[i]);
        }
        net[i] = net[i]+alpha * delta;
    }
    //第二步
    #pragma omp parallel
    #pragma omp for
    for (int i = 0;i < net.size();i++) {
		F32vec4 delta(0, 0, 0, 0);
        for (int j = 0;j < net.size();j++) {
                        delta =delta+ w[i][j] * (net[i] - net[j]);
        }
        net[i] =net[i]+ gamma * delta;
    }
